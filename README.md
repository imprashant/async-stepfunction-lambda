...
```bash
async-stepfunction-lambda$ sam build
```
...
```bash
async-stepfunction-lambda$ sam logs -n StockCheckerFunction --stack-name async-stepfunction-lambda --tail
```
...
```bash
async-stepfunction-lambda$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name async-stepfunction-lambda
```
